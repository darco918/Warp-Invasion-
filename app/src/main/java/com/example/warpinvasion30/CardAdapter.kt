...
class CardAdapter(private val context: Context, private val myModelArrayList: ArrayList<MyModel>) :
    PagerAdapter() {
    ...
    override fun instantiateItem(container: ViewGroup, position: Int): Any {
        ...
        if (position == 0) {
            addWeapons()
            listview.adapter =
                WeaponsListviewAdapter(view.context, R.layout.weapons_list_item, listWeapon)

            listview.setOnItemClickListener { parent, view, position, id ->
                val intent = Intent(context, ItemActivity::class.java)
                intent.putExtra("weaponName", listWeapon[position].title)
                intent.putExtra("image", R.drawable.ic_sniper_rifle)
                context.startActivity(intent)
            }
        } else {
            if (position == 1) {
                addCharacters()
                listview.adapter = CharactersListviewAdapter(
                    view.context,
                    R.layout.characters_list_item,
                    listCharacter
                )

                listview.setOnItemClickListener { parent, view, position, id ->
                    val intent = Intent(context, ItemActivity::class.java)
                    intent.putExtra("weaponName", listCharacter[position].title)
                    intent.putExtra("image", R.drawable.ic_soldier)
                    context.startActivity(intent)
                }
            } else {
                addModes()
                ...
                listview.setOnItemClickListener { parent, view, position, id ->
                    val intent = Intent(context, ItemActivity::class.java)
                    if (position == 0){
                        intent.putExtra("weaponName", "Christmas Special")
                        intent.putExtra("image", R.drawable.ic_christmas_mode)
                    }
                    else{
                        intent.putExtra("weaponName", " Ancient Desert")
                        intent.putExtra("image", R.drawable.ic_gravity)
